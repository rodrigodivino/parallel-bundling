        ...
        const drawLines = () => {
            ...
            const updateAllLinesWithoutAnimation = (selection) => {
                selection
                    ...
                    .style("stroke", d=> this.clusterColor(d[this.clusterOn]))//d=> this.clusterColor(d[this.clusterOn]))
                    .attr('cluster', d=> this.clusterOn === 'all' ? 'all': d[this.clusterOn]);
            ...
            .each(function(key) {
                if(self.domainType[key] !== 'Categorical') d3.select(this).call(d3.axisLeft(y_axes[key]));
                    ...
                    self.domain[key].forEach((category, index) => {
                        ...
                        //g.append('text').text("abc").attr('x', -10).attr('y', self.axesSizesWithPadding[key]/2);
            ...
            .each(function(key) {
                if(self.domainType[key] !== 'Categorical') d3.select(this).call(d3.axisLeft(y_axes[key]));
                    ...
                    g.append('text').text(function() {return d3.select(this.parentNode).attr('category')})
                        ...
                        .attr('x', -4)
                        .attr('y', self.axesSizesWithPadding[key]/2+1)
                        .attr('font-size', '14px')
                        .attr('fill', 'white')
